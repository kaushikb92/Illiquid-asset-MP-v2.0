...
contract TxRegister{
    ...
    function addTx (bytes8 _receiptId, bytes32 _assetTxid, bytes32 _currencyTxid, address _from, address _to, bytes32 _assetID, uint _assetQuantity, uint256 _totalAmount) returns (bool success){
    ...
    function getReceiptIdsByAddress(address _addr) constant returns(bytes8[]){
    ...
    function getReceiptByReceiptId(bytes8 _receiptId) constant returns(bytes32,bytes32,address,address,bytes32,uint,uint256){
    ...
    function getAllTxDetails1() constant returns (bytes32[] , bytes32[], address[] , address[] , bytes32[]) {
    ...
    return(txids1,txids2,senders, receivers, receiptIds);
     ...
     function getAllTxDetails2() constant returns (bytes32[] , uint[] , uint256[]) {
